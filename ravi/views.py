...
def basicdetailhl(request, instance_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            user_details = form.save()
            request.session['mobile_number'] = str(user_details.phone_number)  
            request.session['user_id'] = user_details.id
            request.session['application_id'] = user_details.application_id
            destinationUrl=reverse('personal')
            request.session['plafterurl']=destinationUrl
            request.session['plAppliId']=True
            request.session['orderid'] = user_details.orderid
            request.session['mobile_number']=user_details.phone_number
...
def pldisbursement_details(request, verification_id):
    # Fetch the ApplicationVerification instance based on the verification ID
    verification = get_object_or_404(ApplicationVerification, id=verification_id)
...
def pldisbursement_summary(request):
 ...
 for details in details_list:
        ...
        print(f"Application ID: {details.verification.personal_detail.basicdetailform}")  # Check if this prints the right value

 return render(request, 'admin/pldisbursementview.html', {'details_list': details_list})
